...
class RGBRoundedButtonRenderer(button: GuiButton) : AbstractButtonRenderer(button) {
    override fun render(mouseX: Int, mouseY: Int, mc: Minecraft) {
        var index = 1
        RenderUtils.drawRoundedCornerRect(button.xPosition.toFloat(), button.yPosition.toFloat(), button.xPosition + button.width.toFloat(), button.yPosition + button.height.toFloat(), sqrt((button.width * button.height).toDouble()).toFloat() * 0.1f, (if(button.hovered) { ColorUtils.hslRainbow( index + 1, indexOffset = 100 * 1) } else { Color(60, 60, 60, 150) }).rgb)
        RenderUtils.drawRoundedCornerRect(button.xPosition.toFloat() + 1F , button.yPosition.toFloat() + 1, button.xPosition + button.width.toFloat() - 1, button.yPosition + button.height.toFloat() - 1, sqrt((button.width * button.height).toDouble()).toFloat() * 0.1f, Color(60,60,60,64).rgb)
    }
}
