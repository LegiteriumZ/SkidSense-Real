...
@ModuleInfo(name = "LagReach", category = ModuleCategory.COMBAT)
class LagReach : Module() {
    ...
    private val incomingBlink = BoolValue("IncomingBlink", true)
    private val velocityValue = BoolValue("PauseOnVelocity", true)
    private val outgoingBlink = BoolValue("OutgoingBlink", true)
    private val attackValue = BoolValue("ReleaseOnAttack", true)
    ...
    fun onUpdate(event: UpdateEvent) {
        ...
        if (modeValue.get().equals("fakeplayer", true) || modeValue.get().equals("intave", true)) {
            ...
            currentTarget?.let { current ->
                fakePlayer?.let { currentFaker ->
                    ...
                    when (modeValue.get().lowercase()) {
                        ...
                        "fakeplayer" -> if (pulseTimer.hasTimePassed(pulseDelayValue.get().toLong())) {
                            currentFaker.rotationYawHead = current.rotationYawHead
                            currentFaker.renderYawOffset = current.renderYawOffset
                            currentFaker.copyLocationAndAnglesFrom(current)
                            currentFaker.rotationYawHead = current.rotationYawHead
            ...
            if (maxTimer.hasTimePassed(maxDelayValue.get().toLong())) {
                clearPackets()
                backtrack = false
            }
        }
